...
# --- แบบทดสอบเลือกการตัดสินใจ ---
    ...
    if st.sidebar.button("🎯 แสดงผลการแนะนำ"):
        # กรองข้อมูลตามความชอบ (หลายค่า)
        filtered = [d for d in destinations if d["ประเภท"] in prefer]

        if not filtered:
            st.warning("❌ ไม่มีข้อมูลที่ตรงกับความชอบของคุณ")
        else:
            st.success(f"✨ ผลลัพธ์สำหรับงบประมาณ {budget:,} บาท และความชอบ {', '.join(prefer)}")

            # แนะนำโดยอิงจากจังหวัดเดียวกัน (รวม max_days)
            results = []
            grouped = {}
            for d in filtered:
